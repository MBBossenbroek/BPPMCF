...
def two_step_plain_bin(filename):
    ...
    z = stepone_model.addVars(UB, vtype= GRB.BINARY, name = "z")
    ...
    stepone_model.addConstrs(quicksum(x[i,j]*w[j] for j in range(N)) <= C*z[i] for i in range(UB))
    ...
    for i in range(UB-1):
        stepone_model.addConstr(z[i] >= z[i+1] )
