...
def ILP2star(filename):
    ...
    BPP_model.addConstrs(quicksum(x[i,j,k]* (k+1) for j in range(M) for k in range(max_weight)) <= C*z[i] for i in range(UB))
    ...
    BPP_model.setParam('TimeLimit', 60)
    BPP_model.optimize()
